...
from RickVersion.utils.util_hamil import bilinear_two_mode_H
...
def get_Hamiltonian():
    h_variables = [1 / 2, 1 / 2, 0.6]
    model_H = bilinear_two_mode_H(h_variables)
    return model_H
...
def error_based_costfn(X, H, n, trunc, bd):
    ...
    _, eig_vec = boson_eigenspectrum_sparse(H1, trunc, 1)

    ground_state = eig_vec

    # 3 modes case
    reshaped_gs = ground_state.reshape(trunc, trunc)
...
if __name__ == '__main__':
    ...
    result = minimize(cost_fn, X, method='Powell', options=options, callback=printx)
    ...
    reshaped_gs = ground_state_optim.reshape(truncation, truncation)
